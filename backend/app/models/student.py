...
class StudentRow(BaseModel):
    ...
    Allows extra clinical columns for comprehensive health check-up CSV uploads.
    """
    ...
    parent_name: Optional[str] = Field(default=None, description="Parent / Guardian full name")
    ...
    @classmethod
    def populate_parent_name(cls, data: Any) -> Any:
        if isinstance(data, dict):
            if not data.get("parent_name"):
                data["parent_name"] = data.get("father_name") or data.get("mother_name") or "Parent"
        return data
