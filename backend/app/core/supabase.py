...
class DatabaseService:
    ...
    # In-memory store for fallback/offline OTP handling
    _in_memory_otps: List[Dict] = []
    ...
    def get_student_parent_contact(self, student_id: str) -> Optional[Dict]:
        ...
        try:
            ...
            logger.warning(f"Supabase student contact query failed: {e}. Checking seeds.")

        # Fallback to sample demo seeds
        sample_contacts = {
            "STD-2026-001": {"id": "d0000000-0000-0000-0000-000000000001", "student_id": "STD-2026-001", "parent_phone": "+919876543210", "parent_email": "parent@example.com"},
            "STD-2026-002": {"id": "d0000000-0000-0000-0000-000000000002", "student_id": "STD-2026-002", "parent_phone": "09812345678", "parent_email": "priya.parent@example.com"},
            "STD-2026-003": {"id": "d0000000-0000-0000-0000-000000000003", "student_id": "STD-2026-003", "parent_phone": "09876543210", "parent_email": "rohan.parent@example.com"},
        }
        return sample_contacts.get(student_id)
    ...
    def count_recent_otp_requests(self, contact: str, since_iso: str) -> int:
        ...
        try:
            ...
            logger.warning(f"Supabase count_recent_otp_requests failed: {e}")

        # In-memory check
        count = 0
        for item in self._in_memory_otps:
            if item.get("contact") == contact and item.get("created_at", "") >= since_iso:
                count += 1
        return count
    ...
    def create_otp_request(
        ...
        try:
            ...
            logger.warning(f"Supabase create_otp_request failed: {e}. Storing in memory.")

        self._in_memory_otps.append(record)
        return record
    ...
    def get_latest_active_otp_request(
        ...
        try:
            ...
            logger.warning(f"Supabase get_latest_active_otp_request failed: {e}")

        # In-memory search
        for item in reversed(self._in_memory_otps):
            if (
                item.get("student_id") == student_id
                and item.get("contact") == contact
    ...
    def increment_otp_attempts(self, otp_id: str, current_count: int) -> int:
        ...
        try:
            ...
            pass
        for item in self._in_memory_otps:
            if str(item.get("id")) == str(otp_id):
                item["attempt_count"] = new_count
        return new_count
